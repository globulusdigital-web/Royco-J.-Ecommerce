...
export function serializeAppointment(row) {
  ...
  return {
    ...
    providerType: row.provider_type || row.providerType || "astrologer",
    specialist: row.specialist || "First available",
    consultationMode: row.consultation_mode || row.consultationMode || "in_person",
    service: row.service,
...
export function serializeProduct(row) {
  ...
  return {
    ...
    diamondTier: row.diamond_tier || row.diamondTier || "",
    price,
    compareAtPrice,
