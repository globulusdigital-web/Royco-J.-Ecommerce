...
export default function JyotishiPage() {
  ...
  return <div className="jyotishi-catalog">
    <header className="page-hero jyotishi-catalog-hero"><div className="container-wide"><span className="eyebrow eyebrow-light">Astrological gemstones · Vedic jewellery</span><h1>Jyotishi Jewels,<br /><em>chosen with intention.</em></h1><p>Certified natural gemstones in considered Vedic settings, with optional Abhimantrit energization and personal guidance.</p><Link className="button button-gold" to="/appointments?type=astrologer">Consult an astrologer <ArrowRight /></Link></div><MoonStar /></header>
    <section className="jyotishi-filter-bar container-wide" aria-label="Jyotishi jewel filters">
      {[["gemstone", "Gemstone"], ["sign", "Rashi / Zodiac"], ["planet", "Planet"], ["metal", "Metal setting"]].map(([key, label]) =>
        <label key={key}><span>{label}</span><select value={filters[key]} onChange={(event) => update(key, event.target.value)}>{options(key).map((value) => <option key={value}>{value}</option>)}</select></label>)}
    ...
    <section className="jyotishi-grid container-wide">
      {visible.map((jewel) => <article className="jyotishi-jewel-card" key={jewel.id}>
        <div className="jyotishi-jewel-image"><img src={jewel.image} alt="" /><span><BadgeCheck /> Certified untreated</span></div>
        <div className="jyotishi-jewel-copy"><span className="eyebrow">{jewel.planet} · {jewel.sign}</span><h2>{jewel.name}</h2><dl><div><dt>Gemstone</dt><dd>{jewel.gemstone}</dd></div><div><dt>Weight</dt><dd>{jewel.carat}</dd></div><div><dt>Origin</dt><dd>{jewel.origin}</dd></div><div><dt>Setting</dt><dd>{jewel.metal}</dd></div></dl><div className="jyotishi-card-foot"><strong>{formatMoney(jewel.price)}</strong><span><Sparkles /> Abhimantrit option</span></div></div>
      </article>)}
    ...
    <section className="jyotishi-consult-cta container"><Sparkles /><div><span className="eyebrow">Personal recommendation</span><h2>Unsure which gemstone is right?</h2><p>Book a private reading before purchasing. Gemstone suitability depends on the full birth chart, not a sun sign alone.</p></div><Link className="button button-dark" to="/appointments?type=astrologer">Book an appointment <ArrowRight /></Link></section>
  </div>;
}
